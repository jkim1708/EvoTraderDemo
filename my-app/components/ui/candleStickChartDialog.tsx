...
function getIndexStartDate(preparedData: {
    ...
    const foundIndex = preparedData.findIndex((data) => {
    ...
    if (foundIndex != -1) {
        ...
        if (foundIndex === -1) {
            ...
            //look two days before the start date
            const startDate = new Date(props.strategy.backtestingOffSample.startDate);
            const twoDaysBefore = new Date(startDate.getTime() - 2 * 24 * 60 * 60 * 1000);
            const foundTwoDaysPriorIndex = preparedData.findIndex((data) => {
                return (data.ts.split(',')[0].trim() === twoDaysBefore.toISOString().split('T')[0].trim())
            });


            if (foundTwoDaysPriorIndex != -1) {
                return foundTwoDaysPriorIndex;
            } else {
                // is still not found find 2 days after
                const twoDaysAfter = new Date(startDate.getTime() + 2 * 24 * 60 * 60 * 1000);
                const foundTwoDaysAfterIndex = preparedData.findIndex((data) => {
                    return (data.ts.split(',')[0].trim() === twoDaysAfter.toISOString().split('T')[0].trim())
                });

                return foundTwoDaysAfterIndex;
    ...
    function getIndexEndDate(preparedData: {
        ...
        if (foundIndex != -1) {
            ...
            //look two days before the start date
            const endDate = new Date(props.strategy.backtestingOffSample.endDate);
            ...
            if (foundTwoDaysPriorIndex != -1) {
                ...
                // is still not found find 2 days after
                const twoDaysAfter = new Date(endDate.getTime() + 2 * 24 * 60 * 60 * 1000);
                const foundTwoDaysAfterIndex = preparedData.findIndex((data) => {
                    return (data.ts.split(',')[0].trim() === twoDaysAfter.toISOString().split('T')[0].trim())
                });
                ...
                <div className={"border rounded p-3"}>
                    ...
                    >
                        ...
                        >
                            ...
                            <YAxis orientation='right' yAxisId="2" dataKey="rsi" domain={['auto', 'auto']}
                                   allowDecimals={true}/>
